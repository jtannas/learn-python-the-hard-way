...
A1: If an argument is provided, it prints it as a prompt for an input.
    Then it takes the user input from the terminal and reads it into its return value.
...
while (user_number <= 10):
    try:
        user_number = int(raw_input('Please input an integer greater than 10: '))
    except ValueError, e:
...
'''
Q4: Related to escape sequences, try to find out why the last line has '6\'2"' with that \' sequence. 
    See how the single-quote needs to be escaped because otherwise it would end the string?
A4: Because it was printed with %r, which does the least amount of formatting it can get away with to print the string.
'''
